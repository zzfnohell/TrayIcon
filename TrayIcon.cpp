...
LPWSTR format_last_error(const wstring& msg)
{
    ...
    DWORD cchMsg = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
                                 ...
                                 dwErrorCode, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), pBuffer, MAX_PATH, nullptr);
...
VOID CALLBACK WaitOrTimerCallback(_In_ PVOID lpParameter, _In_ BOOLEAN TimerOrWaitFired)
{
    if (!TimerOrWaitFired)
    {
        [[maybe_unused]] const BOOL succeeded = PostMessage(kState.kDlg, UWM_CHILDQUIT, NULL, NULL);
        assert(succeeded);
    }
...
void ShowNotificationData(bool on)
{
    ...
    const path image_path = on ? kState.GetOnIconPath() : kState.GetOffIconPath();

    UINT flags = LR_MONOCHROME;
    flags |= LR_LOADFROMFILE;
    const auto icon = static_cast<HICON>(LoadImage(
        nullptr, image_path.c_str(), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), flags));
    nid.hIcon = icon;
    nid.uID = kData.uID;
    nid.hWnd = kData.hWnd;
    nid.uFlags = NIF_ICON;
...
{
    const path app_path = kState.GetAppPath();

    const wstring app_args = kState.GetAppArgs();
...
void StartProcess()
{
    ...
    const path startup_dir = kState.GetAppWorkDir();
    unique_ptr<wchar_t[]> cmd_line = BuildCmdLine();

    unique_ptr<wchar_t[]> env_block = build_env_block(kState.env_map_);
    rc = CreateProcess(nullptr,                    // No module name (use command line)
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    if (kState.RunScript())
    {
        ...
        const auto icon =
            static_cast<HICON>(LoadImage(nullptr, image_path.c_str(), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON),
                                         GetSystemMetrics(SM_CYSMICON), flags));
        kData.hIcon = icon;
...
BOOL OnInitDialog(HWND hWnd)
{
    ...
    auto hIcon = static_cast<HICON>(LoadImage(nullptr, image_path.c_str(), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON),
...
void ShowContextMenu(HWND hWnd)
{
    ...
    if (HMENU h_menu = CreatePopupMenu())
    {
        ...
        TrackPopupMenu(h_menu, TPM_BOTTOMALIGN, pt.x, pt.y, 0, hWnd, nullptr);
        DestroyMenu(h_menu);
    }
}
...
void UpdateInfoTextByConst(LPCWSTR s) { SetWindowText(wMsg, s); }
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        wMsg = CreateWindow(STATIC_CLASS, WND_TITLE, WS_VISIBLE | WS_CHILD, 50, 20, 300, 60, hWnd, NULL, 0, NULL);

        CreateWindow(BUTTON_CLASS, WND_BUTTON_START_TEXT, WS_VISIBLE | WS_CHILD, 80, 120, 100, 20, hWnd,
                     (HMENU)IDC_START, 0, NULL);

        CreateWindow(BUTTON_CLASS, WND_BUTTON_STOP_TEXT, WS_VISIBLE | WS_CHILD, 200, 120, 100, 20, hWnd,
                     (HMENU)IDC_STOP, 0, NULL);
        OnInitDialog(hWnd);
        break;
