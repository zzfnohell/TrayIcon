...
optional<tuple<wstring, wstring>> try_match(const wstring& s) noexcept
{
    auto pos = s.find(L'=');
...
bool iequals(const std::wstring_view& a, const std::wstring_view& b) noexcept
{
    if (a.size() != b.size())
    {
        return false;
...
void merge_env(list<wstring>& env_list, const list<wstring>& config_list) noexcept
{
...
void parse_env(list<wstring>& env_list) noexcept
{
...
unique_ptr<wchar_t[]> env_list_to_block(const EnvMap& env) noexcept
{
...
bool split_env_pair(const std::wstring& s, std::wstring& name, std::wstring& value) noexcept
{
    size_t pos = s.find(L'=');
    if (pos == std::wstring::npos)
    {
...
EnvMap env_list_to_map(const std::list<std::wstring>& env_list) noexcept
{
    EnvMap env;
...
void merge_env_map(EnvMap& env, const EnvMap& cfg) noexcept
{
...
EnvMap config_map_from_user_map(const std::map<std::wstring, std::wstring>& replacement) noexcept
{
    EnvMap cfg;
...
unique_ptr<wchar_t[]> build_env_block(const map<wstring, wstring>& replacement) noexcept
{
