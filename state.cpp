...
inline void DebugPrint(const std::wstring& s) { OutputDebugString(s.c_str()); }
...
const path& CState::GetAppPath() const { return app_path_; }

void CState::SetAppPath(const std::filesystem::path& val) { app_path_ = canonicalize(val); }

const path& CState::GetOnIconPath() const { return on_icon_path_; }

void CState::SetOnIconPath(const std::filesystem::path& val) { on_icon_path_ = canonicalize(val); }

const path& CState::GetOffIconPath() const { return off_icon_path_; }

void CState::SetOffIconPath(const std::filesystem::path& val) { off_icon_path_ = canonicalize(val); }

const path& CState::GetAppWorkDir() const { return work_dir_; }

void CState::SetAppWorkDir(const std::filesystem::path& val) { work_dir_ = canonicalize(val); }

bool CState::GetAppHide() const { return app_hide_; }

void CState::SetAppHide(bool val) { app_hide_ = val; }

const wstring& CState::GetAppArgs() const { return app_args_; }

void CState::SetAppArgs(const std::wstring& val) { app_args_ = val; }
